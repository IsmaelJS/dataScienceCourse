...
data <- data %>%
  ...
  mutate(UnknownNumberOfDependents = is.na(NumberOfDependents)) %>%
  mutate(UnknownMonthlyIncome = is.na(MonthlyIncome)) %>%
  ...
  mutate(ZeroDebtRatio = ifelse(DebtRatio == 0, TRUE, FALSE)) %>%
  mutate(DebtRatio = ifelse(UnknownMonthlyIncome == TRUE, 0, DebtRatio)) %>%
  mutate(LogDebt = log(MonthlyIncome * DebtRatio)) %>%
  mutate(LogDebt = ifelse(is.infinite(LogDebt), 0, LogDebt)) %>%
  select(-X)
